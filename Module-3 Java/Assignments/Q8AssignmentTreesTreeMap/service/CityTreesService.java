package Q8AssignmentTreesTreeMap.service;

import Q8AssignmentTreesTreeMap.dao.CityTreesDAO;
import java.util.*;
...
public class CityTreesService {
    ...
    public void addTreeToCity() {
        ...
        if (dao.addTreeToCity(city, tree)) {
    ...
    // 🔥 OPTION 6
    public void findCitiesByTree() {

        System.out.print("Enter Tree Name: ");
        String tree = sc.next();

        List<String> cities = dao.findCitiesByTree(tree);

        if (cities.isEmpty()) {
            System.out.println("No cities found with this tree.");
        } else {
            System.out.println("Tree found in cities: " + cities);
        }
    }
}
